...
#	SETUP_CMAKE_SETTINGS_RUN=1
	...
	msys)
		...
		#find -name "link.txt" | xargs sed -i -E "s#^\"(.+?)\"#\1#g" || true #first we strip any quotes off the initial param then we try to quote to bash, if it has more quotes inside the commmands this likely fails
		find -name "link.txt" | xargs sed -i -E "s#^\"(.+?)\"(.+)\$#$BASH_EXEC -c \"\1\2\"#g" || true #first we strip any quotes off the initial param then we try to quote to bash, if it has more quotes inside the commmands this likely fails
