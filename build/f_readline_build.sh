...
BLD_CONFIG_BUILD_NAME="readline";
BLD_CONFIG_CONFIG_CMD_ADDL="--enable-static --disable-shared" #--disable-nls --enable-static
...
function ourmain() {
	startcommon;
	export CFLAGS="-D__WIN32__ $CFLAGS"
if test 5 -gt 100; then
		echo "Just move the fi down as you want to skip steps, or pass the step to skip to (per below) as the first arg"
fi
	if [[ -z $SKIP_STEP || $SKIP_STEP == "checkout" ]]; then
		git clone --recurse-submodules https://git.savannah.gnu.org/git/readline.git .
